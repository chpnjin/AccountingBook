<template>
  ...
  <div class="subsection">
    ...
    <div class="button-container">
      ...
      <button id="sub-active" @click="switchActive" :disabled="btnDisableSub">
      {{ selectedSub.active == 1 ? '停用' : '啟用'}}
      </button>
    </div>
...
const switchActive = async () => {
  accountService.setActive(selectedSub.value).then((response) => {
  obj_TSub.value.updateData([response]);
  });
};
...
const handleSaveSub = (model) => {
  accountService.editSubAccount(model).then((response) => {
    obj_TSub.value.updateData([response]);
  });
};
